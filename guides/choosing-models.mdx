...
- **Title generation** — `1` call (`google/gemini-2.5-flash`, first message of a new conversation only)

This gives **`2N + 2`** calls for the first message and **`2N + 1`** calls for every follow-up message in the same conversation. With the default four-model council, that is **10 calls** on the opening message and **9 calls** on each subsequent one. Frontier models can cost several cents per call, so a council of four premium models may cost $0.20–$0.50 per question. Swapping one or two members for capable mid-tier models is a practical way to reduce costs without dramatically reducing quality.
